  ...
  return (
    <nav className="fixed z-10 flex justify-between w-full px-8 py-2 top-4">
      <div className="flex items-center gap-2 text-white">
        ...
        <p className="text-xl text-white">Assemble</p>
      </div>
      <ul className="flex items-center text-slate-800 font-semibold">
        <li className="px-2 text-md">
          <Link href="/pods">Product</Link>
        </li>
        <li className="px-2 text-md">
...
function NavbarScroll({ isScrolling }) {
  return (
    <motion.nav
      ...
      className="fixed z-10 flex justify-between px-2 py-2 rounded-full ts-bg -translate-x-1/2 left-1/2 top-10 bg-slate-800"
    >
      <ul className="flex items-center">
        ...
        <li className="px-4 py-2 ml-2 text-yellow  text-md">
